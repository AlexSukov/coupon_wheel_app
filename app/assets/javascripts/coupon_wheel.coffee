...
@second_spin = (settings, slices)->
  body_prepend(settings, 'https://3a9f2474.ngrok.io')
  $('#email-form').remove()
  $("
...
$ ->
    ...
    $.ajax
      type: 'POST'
      url: "https://3a9f2474.ngrok.io/clientside"
      ...
      success: (data) ->
        ...
        if getCookie('coupon_wheel_app_facebook') == ''
          $("form[action*='/cart'] [name='checkout']").one 'click', (e) ->
            $this = $(this)
            e.preventDefault()
            show_facebook_sharer(settings, 'https://3a9f2474.ngrok.io')
          ...
          if settings.enable
            if permitted_url
              ...
              $('body').on 'submit', '#email-form', (e) ->
                ...
                $.ajax
                  type: 'POST'
                  url: "https://3a9f2474.ngrok.io/collected_emails"
